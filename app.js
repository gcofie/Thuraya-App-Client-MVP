...
window.bk_generateSlots = async function() {
    ...
    try {
        ...
        if (!slots.length) {
            // GROUP BOOKING HANDOFF:
            // If the group module is active and no same-time slot can hold the whole group,
            // do not show the old dead-end message. Trigger the split-option planner instead.
            const groupSize =
                Number(window.grp_groupSize || window.grp_size || window.grpGroupSize || 0);

            const looksLikeGroupFlow =
                groupSize > 1 ||
                document.getElementById('screen-group-datetime')?.classList.contains('active') ||
                document.getElementById('screen-group-size')?.classList.contains('active') ||
                document.getElementById('screen-group-services')?.classList.contains('active');

            if (looksLikeGroupFlow) {
                const availableTechCount = Array.isArray(bk_techs) ? bk_techs.length : techsToCheck.length;

                slotsEl.innerHTML = `
                    <div class="group-split-prompt" style="grid-column:1/-1;background:#fff8e7;border:1.5px solid #f59e0b;border-radius:12px;padding:16px;text-align:left;">
                        <p style="font-weight:700;color:#b45309;margin:0 0 6px;">
                            ⚠️ Not enough technicians for the whole group at one time
                        </p>
                        <p style="color:#78350f;margin:0 0 12px;font-size:0.88rem;line-height:1.5;">
                            We could not find a same-time slot for your full group on this date.
                            You can choose how to split the group instead.
                        </p>
                        <button class="btn-primary full" type="button"
                            onclick="window.grp_showSplitOptionsFromSoloEngine && window.grp_showSplitOptionsFromSoloEngine('${date}', ${groupSize || 2}, ${availableTechCount || 1})">
                            Choose split options
                        </button>
                        <div id="grp_inlineSplitOptions" style="margin-top:14px;"></div>
                    </div>
                `;

                if (typeof window.grp_showSplitOptionsFromSoloEngine === 'function') {
                    window.grp_showSplitOptionsFromSoloEngine(date, groupSize || 2, availableTechCount || 1);
                } else if (typeof window.grp_showSplitPlanner === 'function') {
                    window.grp_showSplitPlanner(date, '09:00', Math.max(1, availableTechCount || 1));
                } else if (typeof window.showGroupSplitOptions === 'function') {
                    window.showGroupSplitOptions(groupSize || 2, Math.max(1, availableTechCount || 1), date);
                }
...
// ============================================================
// GROUP BOOKING FALLBACK BRIDGE
// Allows app.js solo slot engine to trigger split options if the
// group module did not expose a matching handoff function.
// Safe: does not affect normal solo booking.
// ============================================================
window.grp_showSplitOptionsFromSoloEngine = window.grp_showSplitOptionsFromSoloEngine || function(dateStr, groupSize, availableTechCount) {
    groupSize = Number(groupSize || window.grp_groupSize || window.grp_size || 2);
    availableTechCount = Math.max(1, Number(availableTechCount || (Array.isArray(bk_techs) ? bk_techs.length : 1)));

    function generateSplits(total, maxSize) {
        const results = [];
        function walk(remaining, path, last) {
            if (remaining === 0) {
                results.push(path);
                return;
            }
            const upper = Math.min(maxSize, remaining, last);
            for (let n = upper; n >= 1; n--) {
                walk(remaining - n, path.concat(n), n);
            }
        }
        walk(total, [], maxSize);
        return results;
    ...
    const splits = generateSplits(groupSize, Math.min(groupSize, availableTechCount));
    const target =
        document.getElementById('grp_inlineSplitOptions') ||
        document.getElementById('grp_splitOptions') ||
        document.getElementById('splitOptions') ||
        document.getElementById('bk_slots');

    if (!target) {
        toast('Split options are available, but the split container was not found.', 'warning');
        return;
    }

    const recommended = splits[0] || [groupSize];

    target.innerHTML = `
        <div class="group-split-options" style="display:flex;flex-direction:column;gap:10px;">
            <p style="font-weight:700;color:var(--primary);margin:0 0 4px;">
                Choose how to split your group of ${groupSize}
            </p>
            <p style="font-size:0.82rem;color:var(--text-muted);margin:0 0 8px;">
                Available technicians per slot: ${availableTechCount}. Recommended: <strong>${recommended.join(' + ')}</strong>
            </p>
            ${splits.map((s, i) => `
                <button class="btn-outline full" type="button"
                    onclick="window.grp_acceptSplitFromSoloEngine && window.grp_acceptSplitFromSoloEngine('${dateStr}', '${s.join('-')}')">
                    ${i === 0 ? 'Recommended — ' : ''}${s.join(' + ')}
                </button>
            `).join('')}
        </div>
    `;
...
window.grp_acceptSplitFromSoloEngine = window.grp_acceptSplitFromSoloEngine || function(dateStr, splitStr) {
    const split = splitStr.split('-').map(Number).filter(Boolean);
    window.grp_selectedSplit = split;
    window.grp_splitDate = dateStr;

    if (typeof window.grp_renderSplitSchedule === 'function') {
        window.grp_renderSplitSchedule(dateStr, split);
        return;
    }

    if (typeof window.grp_showSplitPlanner === 'function') {
        window.grp_showSplitPlanner(dateStr, '09:00', Math.max(...split));
        return;
    }

    toast('Split selected: ' + split.join(' + ') + '. Continue with the group split flow.', 'success');
};
