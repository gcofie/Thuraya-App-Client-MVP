...
// ============================================================
// THURAYA DATE FIELD ENHANCEMENT — MOBILE ONLY + DESKTOP NATIVE
// Scope: profile DOB, account DOB, individual booking date, group booking date.
// Mobile: custom visible display + native picker tap target.
// Desktop: native input remains fully clickable/tabbable.
// ============================================================
(function(){
    const DATE_INPUT_IDS = ['prof_dob', 'acct_dob', 'bk_date', 'grp_date'];

    function th_isTouchDateMode(){
        return (
    ...
    function th_syncDateDisplay(input){
        const field = input?.closest('.th-date-field');
        const text = field?.querySelector('.th-date-display-text');
        const display = field?.querySelector('.th-date-display');
    ...
    function th_enhanceOneDateInput(input){
        ...
        if ((input.id === 'bk_date' || input.id === 'grp_date') && !input.min) input.min = todayStr;
        if ((input.id === 'prof_dob' || input.id === 'acct_dob') && !input.max) input.max = todayStr;

        if (!th_isTouchDateMode()) {
            th_resetDesktopDateInput(input);
            return;
        }
        ...
        input.classList.add('th-date-native', 'th-mobile-date-input');
        input.dataset.thDateEnhanced = 'mobile';

        if (!input.dataset.thDateEventsBound) {
            input.addEventListener('change', function(){ th_syncDateDisplay(input); });
            input.addEventListener('input', function(){ th_syncDateDisplay(input); });
            input.dataset.thDateEventsBound = 'true';
        }
        if (!display.dataset.thDateEventsBound) {
            display.addEventListener('click', function(e){
                e.preventDefault();
                e.stopPropagation();
                th_openNativeDatePicker(input);
            });
            display.dataset.thDateEventsBound = 'true';
        }
    ...
    document.addEventListener('click', function(){ setTimeout(window.thurayaEnhanceMobileDateFields, 60); });
    document.addEventListener('change', function(e){ if (e.target?.matches?.('input[type="date"]')) th_syncDateDisplay(e.target); });
    window.addEventListener('resize', function(){ setTimeout(window.thurayaEnhanceMobileDateFields, 120); });
})();
// === END THURAYA DATE FIELD ENHANCEMENT ===
